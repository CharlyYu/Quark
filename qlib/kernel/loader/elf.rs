...
pub fn MapSegment(
    ...
    let addr = if endMem.0 - startMem.0 == 0 {
        ...
        info!("virtual address is {:x}, fileoffset is {:x}, delta is {:x}, offset is {:x}, len is {:x}, startMem is {:x}",
            header.virtual_addr, header.offset, header.virtual_addr - header.offset, offset, endMem.0 - startMem.0, startMem.0);
